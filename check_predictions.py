...
def check_predictions(sim_data_file: Path, prediction_file: Path, output_file: str):
    ...
    for i, prediction in enumerate(predictions):
        ...
        stat_results.append([
            ...
            str(first_score),
            str(selected_score),
            str(first_score-selected_score)
        ])

    plain_writer = Path(output_file).open("w", encoding="utf8")
    tsv_writer = Path(output_file + ".tsv").open("w", encoding="utf8")
    tsv_writer.write("\t".join(["ID", "Best", "First", "Selected", "Score_First", "Score_Selected", "Score_Diff"]) + "\n")

    for result in stat_results:
        plain_writer.write(f"ID            : {result[0]}\n")
        plain_writer.write(f"Best          : {result[1]}\n")
        plain_writer.write(f"First         : {result[2]}\n")
        plain_writer.write(f"Selected      : {result[3]}\n")
        plain_writer.write(f"Score First   : {result[4]}\n")
        plain_writer.write(f"Score Selected: {result[5]}\n")
        plain_writer.write(f"Score Diff    : {result[6]}\n\n\n")
...
if __name__ == "__main__":
    ...
    check_predictions(args.sim_data_file, args.prediction_file, args.output_file)
